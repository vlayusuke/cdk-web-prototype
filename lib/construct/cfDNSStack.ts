...
import { aws_certificatemanager as acm, aws_elasticloadbalancingv2 as elbv2, aws_route53 as route53, aws_route53_targets as route53_targets } from 'aws-cdk-lib';
...
export class cfDNSStack extends Construct {
    ...
    constructor(scope: Construct, id: string, props: commonProps, pocProps: pocProps) {
        super(scope, id);

        // ------------------------------------------------------------
        // Application Load Balancer Configuration
        // ------------------------------------------------------------
        this.albExternal = new elbv2.ApplicationLoadBalancer(this, 'albExternal', {
            vpc: pocProps.Vpc,
            internetFacing: true,
        });

        cdk.Tags.of(this.albExternal).add('Name', `${props.projectName}-${props.envName}-alb-external`);
        ...
        this.route53ARecord = new route53.ARecord(this, 'route53ARecord', {
            ...
            target: route53.RecordTarget.fromAlias(new route53_targets.LoadBalancerTarget(this.albExternal)),
        });

        cdk.Tags.of(this.route53ARecord).add('Name', `${props.projectName}-${props.envName}-r53-a-record`);
        ...
        // ------------------------------------------------------------
        this.route53AAAARecord = new route53.AaaaRecord(this, 'route53AAAARecord', {
            zone: this.route53PublicHostedZone,
            recordName: `${props.envName}.${props.nakedDomainName}`,
            target: route53.RecordTarget.fromAlias(new route53_targets.LoadBalancerTarget(this.albExternal)),
        });

        cdk.Tags.of(this.route53AAAARecord).add('Name', `${props.projectName}-${props.envName}-r53-aaaa-record`);
